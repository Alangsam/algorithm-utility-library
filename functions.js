...
function findLongestWordLength(str) {
    let arr = str.split(" ");
    let newStr = arr.reduce((a, b) => {return a.length > b.length ? a : b; });
...
function largestOfFour(arr) {
    let newThing = []
      for (let nums = 0; nums < arr.length; nums += 1) {
        newThing.push(arr[nums].reduce((a,b) => {return a > b ? a: b;}));
...
function titleCase(str) {
    ...
    for (let nums = 0; nums < newList.length; nums +=1) {
      largeLet.push(newList[nums].replace(newList[nums].charAt(0), newList[nums].charAt(0).toUpperCase()))
...
function chunkArrayInGroups(arr, size) {
    let tempArray = []
    for (let nums = 0; nums < arr.length; nums += size) {
